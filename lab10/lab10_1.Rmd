...
elephants %>% 
  ...
  ggplot(aes(x=age, y=height))+geom_point(alpha = 0.5, col = "cornflowerblue", size = 0.5)
```
...
```{r}
elephants %>% 
  ggplot(aes(x=age, y=height)) + geom_point() + geom_smooth(method=lm, se=F)+ #making a best fit line.
...
life_history %>% 
  ...
  geom_point(col = "cornflowerblue", na.rm=T)
```
