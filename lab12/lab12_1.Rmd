...
names
```
#getting rid of the space between yolo" ".csv
```{r}
names_list <- strsplit(names, split = " .csv")
names_list
```
#unlist and put in a vector
```{r}
names_vec <- unlist(names_list)
names_vec
```
What is vec short for? vector

#this is to make it so it lists only the names.
```{r}
...
## Practice
1. Which type of join is most appropriate in this case?  Does it matter? very confused.
```{r}
