...
you don't need quotes on the column names here. think of them as an object of the object which is the dataframe.
```{r}
...
you only write minus or not minus. you never do both. no quotes.
```{r}
...
7. Select the columns that include "mass" as part of the name. need quotes here!! you are looking at part of the column(object) name 
```{r}
select(mammals_new, contains("mass"))
```

8. Select all of the columns that are of class `character`.  what is the difference between these two?
```{r}
