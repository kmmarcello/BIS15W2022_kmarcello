...
```{r}
life_expectancy_long %>% 
  filter(year, between(year, 1920, 2020)) #I do not know how to pull these out.
  group_by(country) %>% 
...
```{r}
life_expectancy_long %>%
  #filter(year, between(year, 1920, 2020))
  group_by(country) %>% 
  summarize(mean_life_expectancy = mean(life_expectancy)) %>% 
  arrange(desc(mean_life_expectancy))
```
...
```{r}
population_longer %>% 
  #exponential growth from a plot
```
...
```{r}
gdp_growth_per_person <- income_per_person %>% 
  #you need to find the highest and lowest numbers per person and subtract them.
```

```{r}
gdp_growth_per_person %>% 
  ggplot(aes())
